...
class MyPostsAdapter(var context: Context) : RecyclerView.Adapter<MyPostsAdapter.ViewHolder>() {
    ...
    fun deletePost(id: String) {
        Fuel.get(Utils.API_DELETE_POST.plus(id))
            .header(
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        return ViewHolder(
            LayoutInflater.from(parent.context)
                .inflate(R.layout.custom_post_row, parent, false)
        )
    }
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        if (query == null) {
            ...
            if ((_list[position] as JSONObject).has("services")) {
                ...
                for (items in 0 until services.length()) {
                    if (items == services.length() - 1) {
                        ...
                        tags =
                            tags.plus(services.optJSONObject(items).optString("name").plus(", "))
            ...
            holder.itemView.custom_post_delete.setOnClickListener {
                deletePost((_list[position] as JSONObject).optString("_id"))
            ...
            holder.itemView.custom_post_delete.setOnClickListener {
                deletePost((list[position] as JSONObject).optString("_id"))
                //this.removeItem_(position)
            }
        }
    }
